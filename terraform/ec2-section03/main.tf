...
resource "aws_ebs_volume" "default-vol" {
  availability_zone = "sa-east-1"
  size = 1 # Size of the volume in gibibytes (GiB).
  type = "gp2"
}

resource "aws_volume_attachment" "t2-nano-inst-volume" {
  device_name = "/dev/ssh"
  instance_id = aws_instance.t2-nano-inst.id
  volume_id = aws_ebs_volume.default-vol.id
