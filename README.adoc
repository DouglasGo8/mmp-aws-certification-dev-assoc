...
- https://medium.com/@pra4mesh/aws-lambda-in-java-and-terraform-544da9102e37

- https://adamtheautomator.com/terraform-aws-lambda/

- https://github.com/nasa/cumulus/blob/master/example/cumulus-tf/lambdas.tf
...
* Lambda using JVM/Native Image (GraalVM) with Quarkus/Apache Camel, API Gateway and DynamoDb over Terraform IaC, with only a GET method GET method
...
IMPORTANT: *_AWS Lambda with Camel Quarkus and Terraform_*
...
* We can customize all lifecylcle in gateway, [Method Request (Auth, Header, etc) - Integration Request, Integration Response Method Response]
