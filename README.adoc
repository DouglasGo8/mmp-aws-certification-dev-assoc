...
aws ecr get-login-password \
  ...
  {{AWS_CREDENTIAL_ID}}.dkr.ecr.sa-east-1.amazonaws.com
docker tag myapp {{AWS_CREDENTIAL_ID}}.dkr.ecr.sa-east-1.amazonaws.com/aws-dev-repo:myapp

# docker run --rm --name myapp -p 8080:8080 myapp # only local test purpose - must be remove after test
docker push {{AWS_CREDENTIAL_ID}}.dkr.ecr.sa-east-1.amazonaws.com/aws-dev-repo:myapp
----
...
>
> > *Classic Load Balancer* (Layer 4 and Layer 7) **deprecated** note recomended by AWS
>
...
** Most Popular and frequently used ELB in AWS, support WebSockets and HTTP/HTTPs over Layer 7, with support all import lb feat, and *automatically* scaling
based on demand
** Can load balance between
  . EC2 Instances
  . Containerized apps (Amazon ECS)
  . Web apps (using IP address)
  . Lambda can be used too, but is no used frequentely most common approach in lambda is API Gateway
** Target Groups
...
Quarkus,Camel/Fargate/ECR/ALB/Terraform
...
https://github.com/turnerlabs/terraform-ecs-fargate/tree/master/env/dev

push to 501317491826.dkr.ecr.sa-east-1.amazonaws.com/aws-dev-repo